...
create_data_for_model <- function(spec_graph,grade_info = grades_3, hw_num=1){
  ...
  if(TRUE){
    ...
    for(mem in members){
      ...
      for(num in 1:length(neigh_vec)){
        if(length(neigh_vec[[num]]) > 0){
          df[df$code == mem,names(neigh_vec)[num]] <- mean(df[neigh_vec[[num]],
                                                              paste("hw", hw_num,sep="")]) - mean(df[,paste("hw", hw_num,sep="")])
      ...
      #if(length(neigh_in) > 0){
      #  df$Ave_Score_of_Helpers[df$code == mem] <- mean(df[neigh_in,paste("hw", hw_num,
      #                                                                  sep="")])
      #}
      #if(length(neigh_out) > 0){
      #  df$Ave_Score_of_Helpees[df$code == mem] <- mean(df[neigh_out,paste("hw", 
      #                                                           hw_num,
      #                                                           sep="")])
      #}
    }
...
Try using MCMC to get parameter estimates for each person's ability and for the effect of collaboration
...
If i just restrict it to Homeworks, then I can get some results
I think this is coming from the difficulty in encoding tests in the same format as homeworks. 

It is also possible that this is because tests are not impacted in the same way that homeworks are. Maybe the best predcitors of test grades is just how well you did on homeworks regardless of how you achived that. 
...
flattened$Code <- as.factor(flattened$Code)
lm1 <- lm(data = flattened, 
          Grade ~ Ave_Score_of_Helpers*In_Degree + Code +  Ave_Score_of_Recip*Recip_Degree + 
            Ave_Score_of_Helpees*Out_Degree)
summary(lm1)
mean((predict(lm1, newdata = flattened[!(1:(9*110) <= 8*110),]) - flattened[!(1:(9*110) <= 8*110),"Grade"])**2)
mean((tapply(flattened$Grade[(1:(9*110) <= 8*110)],flattened$Code[(1:(9*110) <= 8*110)],mean) - flattened[!(1:(9*110) <= 8*110),"Grade"])**2)
...
mu1 <- c(lm1$coefficients[1],lm1$coefficients[1] +lm1$coefficients[grepl("Code",names(lm1$coefficients))]) 
```
...
#boxplot(flattened_mean$Grade[thing_for_subsetting<7]~flattened_mean$In_Degree[thing_for_subsetting<7])
#boxplot(flattened_mean$Grade[thing_for_subsetting<7]~flattened_mean$Out_Degree[thing_for_subsetting<7])
```
...
barplot(c(error_lm_total/ifelse(skip_test,7,9),
...
This hsould probably be moved down to the lower section
...
```{r fitting with growth didnt work}
...
for(i in 1:n_folds){
  ...
  ability_0 <- tapply(flattened_mean$Grade[subset_for_this],
...
Don't do tests for this part because I am not learning anythign about them

"Grade ~ Out_Degree\*Ave_Score_of_Helpees + In_Degree \* Ave_Score_of_Helpers" doesn't work super well
...
names(to_plot) <- c("Baseline","Random Forest:\n Only Network",
