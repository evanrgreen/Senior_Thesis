...
num_full_students <- num_full_students
thing_for_subsetting <- 0:(9 * num_full_students - 1) %/% num_full_students
mean(flattened$Out_Degree[flattened$In_Degree == 0 & thing_for_subsetting < k_num_psets] == 0)
```
...
summary(lm1)
mean((predict(lm1, newdata = flattened[!(1:(9 * num_full_students) <= 8 * num_full_students), ]) -
  flattened[!(1:(9 * num_full_students) <= 8 * num_full_students), "Grade"]) ** 2)
mean((tapply(
  flattened$Grade[(1:(9 * num_full_students) <= 8 * num_full_students)],
  flattened$Code[(1:(9 * num_full_students) <= 8 * num_full_students)], mean
) -
  flattened[!(1:(9 * num_full_students) <= 8 * num_full_students), "Grade"]) ** 2)
```
...
for (i in 1:k_num_psets) {
  mu1 <- predict(lm1, newdata = flattened[0:(9 * num_full_students - 1) %/% num_full_students == (i - 1), ])
...
print("test")
for (i in 1:2) {
  mu1 <- predict(lm1, newdata = flattened[0:(9 * num_full_students - 1) %/% num_full_students == (i + 6), ])
  print(i)
...
p <- ggplot(
  flattened_mean[thing_for_subsetting < k_num_psets & flattened_mean$In_Degree < 4, ],
...
p <- ggplot(for_plot[thing_for_subsetting < k_num_psets, ], aes(factor(Out_Degree), Grade))
...
barplot(
  c(
    error_lm_total / ifelse(skip_test, k_num_psets, 9),
    (error_lm_total + error_dif) / ifelse(skip_test, k_num_psets, 9)
  ),
  col = nice_colors
)
...
flattened_plus_gender <- merge(
  flattened_mean[thing_for_subsetting < ifelse(skip_test, k_num_psets, 9), ],
...
percentile_df <- data.frame(matrix(NA, nrow = num_full_students, ncol = 3))
...
z <- data.frame(matrix(
  ...
  ncol = nrow(graph_data_sets[[k_num_psets]])
))
...
LL <- function(Code_1 = as.double(spar), membership=x, grades=flattened_mean$Grade) {
  # Find residuals
  #
  components <- lapply(membership, function(y) y %*% Code_1[1:num_full_students])
...
spar <- as.double(spar)
components <- lapply(x, function(y) y %*% spar[1:num_full_students])
errors <- flattened_mean$Grade - components[[1]] * spar[111] -
  components[[2]] * spar[112] - components[[3]] * spar[113] - components[[4]]
mean(errors ** 2)
spar <- as.double(spar)
components <- lapply(x, function(y) y %*% a$par[1:num_full_students])
...
for (i in 1:k_num_psets) {
  lm1 <- lm(
    ...
    subset = thing_for_subsetting != (i - 1) & thing_for_subsetting < k_num_psets
  )
  lm_list[[i]] <- lm1
  mu1 <- predict(lm1, newdata = flattened_mean[thing_for_subsetting == (i - 1) &
    thing_for_subsetting < k_num_psets, ])
  rf1 <- randomForest(
    data = flattened_mean,
...
barplot(c(
  error_lm_total / k_num_psets, error_rf_total / k_num_psets,
  (error_lm_total + error_dif) / k_num_psets
), col = nice_colors)

print(error_rf_total / k_num_psets)
print((error_lm_total + error_dif) / k_num_psets)
...
lm1 <- lm(
  ...
  Grade  ~ Constraint + (Recip_Degree) + Big_Component
    + (In_Degree == 0) + (Out_Degree == 0),
  subset = thing_for_subsetting < k_num_psets
)
...
to_plot <- c(
  (error_lm_total + error_dif) / k_num_psets, 22.55918, error_rf_total / k_num_psets,
  13.5, 12.6
)
names(to_plot) <- c(
...
for (i in 3:ncol(flattened_mean)) {
  if (is.numeric(flattened_mean[, i])) {
    tests[, colnames(flattened_mean)[i]] <- rep(tapply(
      flattened_mean[thing_for_subsetting < k_num_psets, i],
      flattened_mean$Code[thing_for_subsetting < k_num_psets],
      mean
    ), 2)
  }
}
tests$Ave_Grade <- rep(tapply(
  flattened_mean[thing_for_subsetting < k_num_psets, "Grade"],
  flattened_mean$Code[thing_for_subsetting < k_num_psets],
  mean
), 2)
lm_t <- stepAIC(lm(
...
just_hw <- thing_for_subsetting < k_num_psets
...
for (i in 1:n_trials) {
  ...
  for (j in 1:(k_num_psets * num_full_students)) {
    a <- ifelse(length(helpers[[j]]) > 0,
      flattened_mean$Trial[flattened$Code %in% helpers[[j]] &
        thing_for_subsetting == (j - 1) %/% num_full_students], 0
    )
